...
namespace sqlite {
	...
	private:
		...
		friend void get_col_from_db(database_binder& db, int inx, date::year_month_day& w);
		friend database_binder& operator <<(database_binder& db, const date::year_month_day& txt);
		//friend void get_col_from_db(database_binder& db, int inx, date::sys_seconds& w);
	...
	public:
		...
		template<typename... Types>
		void operator>>(std::optional<std::tuple<Types...>>& opt) {
			int hresult;
			_start_execute();

			if ((hresult = sqlite3_step(_stmt.get())) == SQLITE_ROW) {
				std::tuple<Types...> tup;
				tuple_iterate<std::tuple<Types...>>::iterate(tup, *this);
				opt.emplace(std::move(tup));
			}
			else opt.reset();
